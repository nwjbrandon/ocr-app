...
resource "aws_lambda_function" "ywaste_textract_ocr_lambda" {
  filename      = "ywaste_textract_ocr_lambda/ywaste_textract_ocr_lambda.zip"
  function_name = "ywaste_textract_ocr_lambda"
  role          = aws_iam_role.ywaste_lambda_iam.arn
  handler       = "lambda_function.lambda_handler"

  source_code_hash = filebase64sha256("ywaste_textract_ocr_lambda/ywaste_textract_ocr_lambda.zip")
...
resource "aws_api_gateway_deployment" "ywaste_apis_deployment" {
  depends_on  = [
    aws_api_gateway_integration.ywaste_create_presigned_post_integration, 
